...
router.post('/new', async (req, res) => {
  try {
    ...
    if (/^\s*$/.test(password)) {
        return res.status(400).json({
          error: 'Se espera valor de passwrd'
        });
    }
    ...
    const newUser = await user.addUsuarios({email, nombre, avatar, password,estado});
...
router.put('/update/:codigo', async (req, res)=>{
  try {
    ...
    return res.status(200).json({updatedUsario:updateResult});
...
router.delete('/delete/:codigo', async (req, res) => {
  try {
    ...
    const usuarioEliminado = await user.deleteUsuario({ codigo: parseInt(codigo)});
