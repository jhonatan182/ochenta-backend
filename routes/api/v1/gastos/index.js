...
const GastoDao = require('../../../../dao/sqlite/models/GastosDao');
...
router.get('/byid/:codigo', async (req, res) => {
    try {
        ...
        const registro = await gasto.getUsuarioById({
...
router.post('/new', async (req, res) => {
    try {
        ...
        const newGasto = await gasto.addGastos({
            type,
            description,
            amount,
            category,
...
router.put('/update/:codigo', async (req, res) => {
    try {
        ...
        if (!/^\d+$/.test(codigo)) {
            return res
                .status(400)
                .json({ error: 'El codigo debe ser un dígito válido.' });
        }
        const {
            type = '',
            description = '',
        ...
        const updateResult = await gasto.updateGasto({
            codigo: parseInt(codigo),
            type,
            description,
            amount,
...
router.delete('/delete/:codigo', async (req, res) => {
    try {
        ...
        const gastoEliminado = await gasto.deleteGasto({
            codigo: parseInt(codigo),
        });
