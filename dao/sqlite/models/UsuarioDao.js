...
module.exports = class UsuarioDao extends DaoObject {
    ...
    setup() {
        if (process.env.SQLITE_SETUP) {
            const createStatement =
                'CREATE TABLE IF NOT EXISTS usuarios (id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT, estado TEXT, password TEXT, nombre TEXT , avatar TEXT, fchaIngreso TEXT);';
    ...
    insertOne({ email, password, nombre, avatar, estado }) {
        ...
        const sqlstr =
            'INSERT INTO usuarios (email, password, nombre, avatar, estado ,fchaIngreso) values (?, ?, ?, ?, ?, ?);';
        const sqlParamArr = [
            email,
            password,
            nombre,
    ...
    updateOne({ codigo, email, password, nombre, avatar, estado }) {
        const sqlstr =
            'UPDATE usuarios set email = ?, password = ?, nombre = ?, avatar = ? , estado = ? where id = ?;';
